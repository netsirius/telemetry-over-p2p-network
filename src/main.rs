...
async fn create_new_doc(name: &str, author: &str, content: &str) -> Result<(), anyhow::Error> {
...
async fn publish_doc(id: usize) -> Result<(), anyhow::Error> {
    let mut local_docs = read_local_docs().await?;
    local_docs
        .iter_mut()
        .filter(|r| r.id == id)
...
async fn read_local_docs() -> Result<Documents, anyhow::Error> {
    let content = fs::read(STORAGE_FILE_PATH).await?;
    let result = serde_json::from_slice(&content)?;
    Ok(result)
}

async fn write_local_docs(docs: &Documents) -> Result<(), anyhow::Error> {
...
async fn main() -> Result<(), Box<dyn Error + Send + Sync + 'static>>{
    ...
    // let span = tracer.start(format!("node_{}", peer_id));
        ...
        if let Some(event) = evt {
            match event {
                EventType::Response(resp) => {
                    ...
                    swarm.behaviour_mut().floodsub.publish(TOPIC.clone(), json.as_bytes()); // publish the response
                }
...
async fn handle_list_peers(swarm: &mut Swarm<DocumentBehaviour>) {
...
async fn handle_list_docs(cmd: &str, swarm: &mut Swarm<DocumentBehaviour>) {
